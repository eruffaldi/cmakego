	...
	foreach(name ${ARGV})
		...
		elseif(${name} STREQUAL NOREQUIRED)
			...
			if(TARGET ${xname})
				...
				elseif(${name} STREQUAL yamlold)
					find_path(YAMLCPPOLD_INCLUDE_DIR yaml-cpp/ostream_wrapper.h)
					find_library(YAMLCPPOLD_LIBRARY yaml-cpp)
					include(FindPackageHandleStandardArgs)
					FIND_PACKAGE_HANDLE_STANDARD_ARGS(YAMLCPPOLD DEFAULT_MSG YAMLCPPOLD_INCLUDE_DIR YAMLCPPOLD_LIBRARY)
					mark_as_advanced(YAMLCPPOLD_INCLUDE_DIR YAMLCPPOLD_LIBRARY)
					if(YAMLCPPOLD_FOUND)
						add_library(p::yamlold INTERFACE IMPORTED)
						set_property(TARGET p::yamlold PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${YAMLCPPOLD_INCLUDE_DIR}")
						set_property(TARGET p::yamlold PROPERTY INTERFACE_LINK_LIBRARIES ${YAMLCPPOLD_LIBRARY})
					elseif(YAMLCPPOLD_LIBRARY)
						message(Fatal YAMLCPP new availble, looking for old)
					endif()		
				elseif(${name} STREQUAL yamlnew)
					find_path(YAMLCPPNEW_INCLUDE_DIR yaml-cpp/ostream.h)
					find_library(YAMLCPPNEW_LIBRARY yaml-cpp)
					include(FindPackageHandleStandardArgs)
					FIND_PACKAGE_HANDLE_STANDARD_ARGS(YAMLCPPNEW DEFAULT_MSG YAMLCPPNEW_INCLUDE_DIR YAMLCPPNEW_LIBRARY)
					mark_as_advanced(YAMLCPPNEW_INCLUDE_DIR YAMLCPPNEW_LIBRARY)
					if(YAMLCPPNEW_FOUND)
						add_library(p::yamlnew INTERFACE IMPORTED)
						set_property(TARGET p::yamlnew PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${YAMLCPPNEW_INCLUDE_DIR}")
						set_property(TARGET p::yamlnew PROPERTY INTERFACE_LINK_LIBRARIES ${YAMLCPPNEW_LIBRARY})
					elseif(YAMLCPPNEW_LIBRARY)
						message(Fatal YAMLCPP old availble, looking for new)
					endif()		
				...
				if(TARGET ${xname})
					# workaround to update a global variable
					GET_PROPERTY(tmp GLOBAL PROPERTY CMAKEGO_LIBRARIESx)
					set(tmp ${tmp} p::${name})
					SET_PROPERTY(GLOBAL PROPERTY CMAKEGO_LIBRARIESx ${tmp})
					# as variable global
					set(CMAKEGO_LIBRARIES ${CMAKEGO_LIBRARIES} ${tmp} PARENT_SCOPE)
					if(${globaluse})
